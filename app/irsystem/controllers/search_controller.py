...
# subways=[]
# for f in os.listdir('app/static/subways'):
# 	subways.append((f.replace('.svg',''), 'static/subways/'+f))
...
def search():
    ...
    return render_template('search.html', name=project_name, first_prototype=v1_link, netid=net_id, query=query, data=data, subways=subways)
